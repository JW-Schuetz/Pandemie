...
datum( end )    % liefert 19.12.2020 (richtig)
sum( neu )      % liefert 2050 (richtig)
neu( end )      % liefert 64 (richtig)

sum( neu( end - 6 : end ) )  % liefert 356 (richtig)

mneu = meanFilter( neu( 1 : end ) );
mneu( end ) * 100000 / einwohnerKreis( lk )     % liefert 368.1375 (richtig)
...
% FIDs Neuinfekte
neu       = tab.NeuerFall;
neuerfall = ( neu == 1 | neu == 0 );   % neuer Fall
tabNeu    = tab( neuerfall, : );

sum( tabNeu.Fallanzahl )    % Fallzahlen insgesamt, liefert 2050 (richtig)

neu       = tab.NeuerFall;
neuerfall = neu == 1;       % neuer Fall
tabNeu    = tab( neuerfall, : );
sum( tabNeu.Fallanzahl )    % liefert 64 (richtig)
